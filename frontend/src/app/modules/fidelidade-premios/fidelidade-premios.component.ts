...
const PREMIOS_COLUNAS: ColunaDef[] = [
  { campo: 'codigo',            label: 'Código',      largura: 90,  minLargura: 60,  padrao: true },
  { campo: 'nome',              label: 'Nome',        largura: 280, minLargura: 120, padrao: true },
  { campo: 'categoria',         label: 'Categoria',   largura: 140, minLargura: 80,  padrao: true },
  { campo: 'pontosNecessarios', label: 'Pontos',      largura: 100, minLargura: 70,  padrao: true },
  { campo: 'estoque',           label: 'Estoque',     largura: 90,  minLargura: 60,  padrao: true },
  { campo: 'ativo',             label: 'Ativo',       largura: 70,  minLargura: 50,  padrao: true },
];

@Component({
  selector: 'app-fidelidade-premios',
  standalone: true,
  imports: [CommonModule, FormsModule, EnterTabDirective],
  templateUrl: './fidelidade-premios.component.html',
  styleUrl: './fidelidade-premios.component.scss'
})
export class FidelidadePremiosComponent implements OnInit, OnDestroy {
  private readonly STATE_KEY = 'zulex_fidelidade_premios_state';
  private readonly STORAGE_KEY_COLUNAS = 'zulex_colunas_fidelidade_premios';

  modo = signal<Modo>('lista');
  premios = signal<Premio[]>([]);
  premioSelecionado = signal<Premio | null>(null);
  premioForm = signal<Premio>(this.novoPremio());
  ...
  private formOriginal: Premio | null = null;
  ...
  private apiUrl = `${environment.apiUrl}/fidelidade/premios`;
  ...
  private async verificarPermissao(acao: string): Promise<boolean> {
    if (this.auth.temPermissao('fidelidade', acao)) return true;
    const resultado = await this.modal.permissao('fidelidade', acao);
  ...
  private readonly TAB_ID = '/erp/fidelidade-premios';
  private fechamentoConfirmado = false;

  ngOnInit() {
    if (!this.tabService.tabs().find(t => t.id === this.TAB_ID)) {
      this.tabService.abrirTab({ id: this.TAB_ID, titulo: 'Catálogo de Prêmios', rota: this.TAB_ID, iconKey: 'box' });
    }
    this.carregar();
  ...
  onKeydown(e: KeyboardEvent) {
    ...
    if (e.key === 'Escape') { if (this.modo() === 'form') { (e as any).__handled = true; if (this.isDirty()) this.cancelarEdicao(); else this.fecharAba(this.abaAtivaId()!); } else if (this.abasEdicao().length > 0) { (e as any).__handled = true; const u = this.abasEdicao()[this.abasEdicao().length - 1]; this.fecharAba(u.premio.id!); } }
    if (e.key === 'F2' && this.modo() === 'lista') { e.preventDefault(); this.editar(); }
    if (e.key === 'Enter' && this.modo() === 'lista' && this.premioSelecionado()) {
    ...
    if ((e.key === 'ArrowDown' || e.key === 'ArrowUp') && this.modo() === 'lista') {
      ...
      e.preventDefault();
      const lista = this.premiosFiltrados();
      if (lista.length === 0) return;
      const atual = this.premioSelecionado();
  ...
  campoAlterado(campo: string): boolean {
    ...
    const atual = (this.premioForm() as any)[campo];
  ...
  private persistirEstado() {
    ...
    sessionStorage.setItem(this.STATE_KEY, JSON.stringify({
      abas: abas.map(a => ({ premio: a.premio, form: a.form, isDirty: a.isDirty })),
    ...
    try {
      ...
      if (state.abas?.length > 0) {
        for (const a of state.abas) {
          if (this.abasEdicao().find(x => x.premio.id === a.premio.id)) continue;
          const novaAba: AbaEdicao = { premio: a.premio, form: this.clonar(a.form), isDirty: a.isDirty };
          this.abasEdicao.update(tabs => [...tabs, novaAba]);
          if (a.premio.id === state.abaAtivaId) {
            this.premioSelecionado.set(a.premio);
            this.premioForm.set(this.clonar(a.form));
            this.formOriginal = this.clonar(a.form);
            this.isDirty.set(a.isDirty);
            this.abaAtivaId.set(a.premio.id);
            this.modoEdicao.set(a.premio.id !== this.NOVO_ID);
            this.modo.set('form');
          }
        }
        return;
      ...
      if (state.abasIds?.length > 0) {
        for (const id of state.abasIds) {
          const f = this.premios().find(x => x.id === id);
  ...
  private restaurarAba(f: Premio, ativar: boolean) {
    if (this.abasEdicao().find(a => a.premio.id === f.id)) return;
    const aba: AbaEdicao = { premio: { ...f }, form: this.clonar(f), isDirty: false };
  ...
  carregar() {
    ...
    this.http.get<any>(this.apiUrl).subscribe({
      ...
      error: (e) => {
        this.carregando.set(false);
        if (e.status === 403) {
          this.modal.permissao('premios', 'c').then(r => {
  ...
  getCellValue(f: Premio, campo: string): string {
    const v = (f as any)[campo];
    if (v === null || v === undefined) return campo === 'estoque' ? '∞' : '—';
    if (typeof v === 'boolean') return v ? 'Sim' : 'Não';
    return String(v);
  }

  selecionar(f: Premio) { this.premioSelecionado.set(f); }
  ...
  restaurarPadrao() {
    this.colunas.set(PREMIOS_COLUNAS.map(c => ({ ...c, visivel: c.padrao })));
    this.salvarColunasStorage();
  }
  ...
  onMouseMove(e: MouseEvent) {
    ...
    const def = PREMIOS_COLUNAS.find(c => c.campo === this.resizeState!.campo);
  ...
  async incluir() {
    ...
    this.salvarEstadoAbaAtiva();
    const jaExiste = this.abasEdicao().find(a => a.premio.id === this.NOVO_ID);
    if (jaExiste) {
      if (jaExiste.isDirty) { this.ativarAba(this.NOVO_ID); this.modoEdicao.set(false); return; }
      else { this.abasEdicao.update(tabs => tabs.filter(t => t.premio.id !== this.NOVO_ID)); }
    }
    const novo = this.novoPremio();
    (novo as any).id = this.NOVO_ID;
    this.premioForm.set(novo);
    this.formOriginal = this.clonar(novo);
    this.modoEdicao.set(false);
    this.isDirty.set(false);
    this.erro.set(''); this.errosCampos.set({});
    this.abaAtivaId.set(this.NOVO_ID);
    const novaAba: AbaEdicao = { premio: { id: this.NOVO_ID, nome: 'Novo cadastro', ativo: true, pontosNecessarios: 0 }, form: this.clonar(novo), isDirty: false };
    this.abasEdicao.update(tabs => [...tabs, novaAba]);
    this.modo.set('form');
  }
  ...
  async editar() {
    ...
    const f = this.premioSelecionado();
    if (!f?.id) return;

    const jaAberta = this.abasEdicao().find(a => a.premio.id === f.id);
    if (jaAberta) {
      this.ativarAba(f.id);
      return;
    }
    ...
    this.premioForm.set(this.clonar(f));
  ...
  ativarAba(id: number) {
    this.salvarEstadoAbaAtiva();
    const aba = this.abasEdicao().find(a => a.premio.id === id);
    if (!aba) return;
    this.abaAtivaId.set(id);
    this.premioForm.set(this.clonar(aba.form));
  ...
  async fecharAba(id: number) {
    ...
    this.abasEdicao.update(abas => abas.filter(a => a.premio.id !== id));
    if (this.abaAtivaId() === id) {
      const restantes = this.abasEdicao();
      if (restantes.length > 0) {
        this.ativarAba(restantes[restantes.length - 1].premio.id!);
      } else {
        this.modo.set('lista');
  ...
  async fecharForm() {
    ...
    if (id != null) {
      this.abasEdicao.update(tabs => tabs.filter(t => t.premio.id !== id));
      this.abaAtivaId.set(null);
    } else {
      this.modo.set('lista');
    }
  ...
  async cancelarEdicao() {
    ...
    const id = this.abaAtivaId();
    if (id === this.NOVO_ID) {
      this.abasEdicao.update(tabs => tabs.filter(t => t.premio.id !== this.NOVO_ID));
      this.abaAtivaId.set(null); this.modo.set('lista'); return;
    }
    if (this.formOriginal) {
      this.premioForm.set(this.clonar(this.formOriginal));
      this.isDirty.set(false);
      if (id) {
        this.abasEdicao.update(abas =>
          abas.map(a => a.premio.id === id ? { ...a, isDirty: false } : a)
        );
      }
    }
  }
  ...
  private salvarEstadoAbaAtiva() {
    ...
    this.abasEdicao.update(abas =>
      abas.map(a => a.premio.id === id
        ? { ...a, form: this.clonar(this.premioForm()), isDirty: this.isDirty() }
        : a
      )
    );
  }
  ...
  async salvar() {
    ...
    const f = this.premioForm();
    if (!f.nome.trim()) erros['nome'] = 'Nome é obrigatório.';
    if (!f.pontosNecessarios || f.pontosNecessarios <= 0) erros['pontosNecessarios'] = 'Pontos devem ser maior que zero.';
    ...
    const body: any = {
      ...
      pontosNecessarios: f.pontosNecessarios,
    ...
    salvarDados$.subscribe({
      next: (r: any) => {
        const premioId = this.modoEdicao() ? f.id! : r.data?.id;
        this.finalizarSalvar(premioId);
      },
      error: () => {
        this.erro.set('Erro ao salvar premio.');
        this.salvando.set(false);
      }
    });
  }

  private finalizarSalvar(premioId: number) {
    this.salvando.set(false);
    this.isDirty.set(false);
    if (this.modoEdicao()) {
      this.fecharAba(premioId);
    }
    this.carregar();
    this.modo.set('lista');
  }
  ...
  async excluir() {
    ...
    const resultado = await this.modal.confirmar(
      'Confirmar Exclusão',
      `Deseja excluir o premio ${f.nome}? O registro será removido permanentemente. Se estiver em uso, será apenas desativado.`,
      'Sim, excluir',
      'Não, manter'
    );
    ...
    this.http.delete<any>(`${this.apiUrl}/${f.id}`, { headers }).subscribe({
      next: async (r: any) => {
        ...
        this.premioSelecionado.set(null);
        this.fecharAba(f.id!);
        this.carregar();
      ...
      error: () => {
        this.excluindo.set(false);
        this.modal.erro('Erro', 'Erro ao excluir premio.');
      }
    });
  }

  // ── Form helpers ───────────────────────────────────────────────────
  private readonly camposSemUpper = new Set(['criadoEm', 'imagemUrl']);

  upd(campo: keyof Premio, v: any) {
    if (typeof v === 'string' && !this.camposSemUpper.has(campo as string)) v = v.toUpperCase();
    this.premioForm.update(f => ({ ...f, [campo]: v }));
    this.isDirty.set(true);
    this.atualizarDirtyAba();
  }

  /** Converte string do input em número (ou null se vazio) — substitui type="number". */
  updNum(campo: keyof Premio, v: string) {
    const limpo = (v ?? '').replace(/\D/g, '');
    const num = limpo === '' ? null : parseInt(limpo, 10);
    this.premioForm.update(f => ({ ...f, [campo]: num as any }));
    this.isDirty.set(true);
    this.atualizarDirtyAba();
  }
  ...
  private atualizarDirtyAba() {
    ...
    if (!id) return;
    this.abasEdicao.update(abas =>
      abas.map(a => a.premio.id === id ? { ...a, isDirty: true } : a)
    );
  }
  ...
  // ── Utils ──────────────────────────────────────────────────────────
  private novoPremio(): Premio {
    return { nome: '', descricao: null, categoria: null, pontosNecessarios: 0, imagemUrl: null, estoque: null, ativo: true };
  }
