...
interface HierarquiaDetalhe {
  ...
  padrao: boolean;
  aplicarAutomatico: boolean;
  descontoAutoTipo?: number;
  buscarMenorValorPromocao: boolean;
  ativo: boolean;
...
const HIERARQUIA_COLUNAS: ColunaDef[] = [
  ...
  { campo: 'aplicarAutomatico', label: 'Autom\u00e1tico', largura: 90, minLargura: 70, padrao: true },
...
const COMPONENTES = [
  ...
  { valor: 6, label: 'Cliente', icone: 'user' },
  { valor: 7, label: 'Conv\u00eanio', icone: 'users' },
  { valor: 8, label: 'Fam\u00edlia', icone: 'box' },
  { valor: 9, label: 'Grupo Principal', icone: 'box' },
  { valor: 10, label: 'Grupo', icone: 'box' },
  { valor: 11, label: 'SubGrupo', icone: 'box' },
  { valor: 12, label: 'Fabricante', icone: 'box' },
  { valor: 13, label: 'Cond. Pagamento', icone: 'dollar' },
  { valor: 14, label: 'Produto', icone: 'pill' },
];
...
@Component({
  selector: 'app-hierarquia-descontos',
  standalone: true,
  imports: [CommonModule, FormsModule, EnterTabDirective],
  templateUrl: './hierarquia-descontos.component.html',
  styleUrl: './hierarquia-descontos.component.scss'
})
export class HierarquiaDescontosComponent implements OnInit, OnDestroy {
  private readonly STATE_KEY = 'zulex_hierarquia_descontos_state';
  private readonly STORAGE_KEY_COLUNAS = 'zulex_colunas_hierarquia_descontos';
  ...
  fAplicarAutomatico = signal(false);
  fDescontoAutoTipo = signal<number | undefined>(undefined);
  fBuscarMenorValorPromocao = signal(false);
  fAtivo = signal(true);
  itens = signal<HierarquiaItem[]>([]);
  colaboradorIds = signal<Set<number>>(new Set());
  convenioIds = signal<Set<number>>(new Set());
  clienteIds = signal<Set<number>>(new Set());
  ...
  // Client autocomplete
  clienteBusca = signal('');
  clienteResultados = signal<LookupItem[]>([]);
  clienteDropdown = signal(false);
  private clienteBuscaTimer: any = null;
  ...
  private apiUrl = `${environment.apiUrl}/hierarquiadescontos`;
  ...
  private async verificarPermissao(acao: string): Promise<boolean> {
    if (this.auth.temPermissao('hierarquia-descontos', acao)) return true;
    const resultado = await this.modal.permissao('hierarquia-descontos', acao);
  ...
  ngOnDestroy() {
    ...
    if (this.clienteBuscaTimer) clearTimeout(this.clienteBuscaTimer);
  }
  ...
  carregar() {
    ...
    this.http.get<any>(this.apiUrl).subscribe({
      ...
      error: (e) => {
        this.carregando.set(false);
        if (e.status === 403) {
          this.modal.permissao('hierarquia-descontos', 'c').then(r => {
  ...
  async editar() {
    ...
    this.http.get<any>(`${this.apiUrl}/${sel.id}`).subscribe({
      next: r => {
        ...
        this.fAplicarAutomatico.set(d.aplicarAutomatico);
        this.fDescontoAutoTipo.set(d.descontoAutoTipo);
        this.fBuscarMenorValorPromocao.set(d.buscarMenorValorPromocao);
        this.fAtivo.set(d.ativo);
        this.itens.set(d.itens ?? []);
        this.colaboradorIds.set(new Set(d.colaboradorIds ?? []));
        this.convenioIds.set(new Set(d.convenioIds ?? []));
        this.clienteIds.set(new Set(d.clienteIds ?? []));
  ...
  async salvar() {
    ...
    const body = {
      nome,
      padrao: this.fPadrao(),
      aplicarAutomatico: this.fAplicarAutomatico(),
      descontoAutoTipo: this.fAplicarAutomatico() ? this.fDescontoAutoTipo() : undefined,
      buscarMenorValorPromocao: this.fBuscarMenorValorPromocao(),
      ativo: this.fAtivo(),
      itens: this.itens(),
      colaboradorIds: Array.from(this.colaboradorIds()),
      convenioIds: Array.from(this.convenioIds()),
      clienteIds: Array.from(this.clienteIds()),
    };
    ...
    salvar$.subscribe({
      ...
      error: () => {
        this.salvando.set(false);
        this.modal.erro('Erro', 'Erro ao salvar hierarquia de descontos.');
      }
    });
  }
  ...
  private resetForm() {
    ...
    this.fAplicarAutomatico.set(false);
    this.fDescontoAutoTipo.set(undefined);
    this.fBuscarMenorValorPromocao.set(false);
    this.fAtivo.set(true);
    this.itens.set([]);
    this.colaboradorIds.set(new Set());
    this.convenioIds.set(new Set());
    this.clienteIds.set(new Set());
    this.accVinculacoes.set(false);
  ...
  // Secao selection for item (componente 3)
  ...
  toggleConvenio(id: number) {
    const s = new Set(this.convenioIds());
    if (s.has(id)) s.delete(id);
    else s.add(id);
    this.convenioIds.set(s);
    this.isDirty.set(true);
  }
  ...
  onClienteBuscaInput(valor: string) {
    ...
    this.clienteBuscaTimer = setTimeout(() => {
      this.http.get<any>(`${environment.apiUrl}/pessoas/pesquisar`, { params: { termo: valor } }).subscribe({
        next: r => {
          this.clienteResultados.set((r.data ?? []).map((x: any) => ({ id: x.id, nome: x.nome })));
  ...
  adicionarCliente(c: LookupItem) {
    const s = new Set(this.clienteIds());
    s.add(c.id);
    this.clienteIds.set(s);
    if (!this.clientes().find(x => x.id === c.id)) {
  ...
  clienteNome(id: number): string {
    return this.clientes().find(c => c.id === id)?.nome ?? `#${id}`;
  }

  fecharClienteDropdown() {
    setTimeout(() => this.clienteDropdown.set(false), 200);
  }
