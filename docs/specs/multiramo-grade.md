...
### Ajustes pós-teste 2c (2026-07-03)
- **Estoque total automático ao abrir o produto**: `MapDetalhe` passa a somar os SKUs quando o produto **tem variações** (não depende mais do flag `ControlaGrade`) — antes só aparecia após re-salvar a grade. O DTO devolve `ControlaGrade = temGrade` (flag OU tem variação).
- **Estoque negativo**: baixa de estoque na venda deixa de travar em 0 (`Math.Max(0,…)`) e passa a respeitar a config **`estoque.permitir.negativo`** (seed **default `true`**). Ligado → estoque fica negativo (inclui o SKU vendido na grade). Desligado → a finalização é **bloqueada** com erro listando os itens sem saldo (validação antes de finalizar). UI: toggle em **Configurações → Estoque**.
- **NFC-e fuso** (não é grade): `dhEmi/dhSaiEnt/dhEvento` agora usam `DataHoraHelper.ComOffset` (offset de Brasília -03:00) — antes o `zzz` pegava o fuso do servidor (UTC no Railway) e a nota saía 3h fora ("emissão atrasada").
