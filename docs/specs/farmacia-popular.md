...
- **RN-03 — Receita obrigatória:** CRM + UF + data de emissão. Data não pode ser superior a **180 dias** (RN-BE-01 do DATASUS — validar localmente antes de enviar).
...
| `pbm.fp.usuario.vendedor` | string | — | Credencial Portal FP — login do operador (MVP único) |
| `pbm.fp.senha.vendedor` | string (encriptada) | — | |
...
| `NuReceita` | string? | — | Opcional (se capturado do prescritor) |
| `PrescritorId` | long? (FK) | — | Reusa tabela Prescritor (SNGPC) se existir o médico |
...
flowchart TD
  ...
  Rec -->|Não| ModalRec[Abre modal: CRM, UF, Data emissão]
  Rec -->|Sim| Fase1
...
| **Usuário (Vendedor)** | `pbm.fp.usuario.vendedor` | Input texto |
| **Senha (Vendedor)** | `pbm.fp.senha.vendedor` | Input password |
...
- **Aba de venda FP** (derivada do caixa-venda/pre-venda):
  - Topo: campos cliente+CPF, CRM, UF, data receita (obrigatórios pra liberar venda).
